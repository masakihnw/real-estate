...
def _diagnose_page(page: "Page", url: str, keywords: list[str]) -> None:
    """ボタン未検出時にページの状態を診断ログに出力する（最大3回）。"""
    global _DIAG_COUNT
    if _DIAG_COUNT >= 3:
        return
    _DIAG_COUNT += 1
    ...
    try:
        ...
        logger.warning(f"[DIAG] URL: {url}")
        logger.warning(f"[DIAG] title={info['title']}, bodyLen={info['bodyLen']}, "
                       f"actualURL={info['url']}")
        if info["elements"]:
            for el in info["elements"]:
                logger.warning(
                    f"[DIAG]  <{el['tag']}> cls={el['cls'][:40]} "
                    f"display={el['display']} vis={el['visibility']} "
                    f"{el['w']}x{el['h']} href={el['href']} "
                    f"text={el['text'][:60]}")
        else:
    ...
    try:
        debug_dir = SCRIPT_DIR / "data" / "browser_debug"
        debug_dir.mkdir(parents=True, exist_ok=True)
        html = page.content()[:50_000]
        url_hash = hashlib.md5(url.encode()).hexdigest()[:8]
        (debug_dir / f"{url_hash}.html").write_text(html, encoding="utf-8")
        logger.warning(f"[DIAG] HTML saved: data/browser_debug/{url_hash}.html")
    except Exception:
        pass
    ...
    try:
        ...
        if property_type != "chuko" and targets:
            ...
            try:
                ...
                if not preflight_btn:
                    logger.warning(
                        f"新築ブラウザ enrichment スキップ: "
                        f"10年後予測ボタンが検出できません（{len(targets)}件）。"
                        f"HTTP enricher の静的データで運用します。")
                    _diagnose_page(page, preflight_url,
                                   ["予測", "10年後", "シミュレーション", "詳細を見る"])
                    _save_json(listings, output_path_p)
                    return
                logger.info("新築プリフライトチェック: OK（ボタン検出成功）")
            except Exception as e:
                logger.warning(f"新築プリフライトチェック失敗: {e}")
                _save_json(listings, output_path_p)
                return
