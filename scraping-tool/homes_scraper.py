...
def _launch_browser():
    ...
    return launch_stealth_browser(referer="https://www.homes.co.jp/")


def _is_waf_or_captcha(html: str) -> bool:
    """HOME'S の WAF/CAPTCHA ページかどうかを判定。"""
    if not html:
        return True
    low = html.lower()
    if "human verification" in low:
        return True
    if "captcha" in low and len(html) < 5000:
        return True
...
def fetch_list_page(context: BrowserContext, url: str, *, max_retries: int = 3) -> str:
    """Playwright でページを取得し、レンダリング済み HTML を返す。

    WAF/CAPTCHA 検知時は指数バックオフでリトライする。
    """
    for attempt in range(max_retries):
        page = context.new_page()
        try:
            logger.info("HOME'S: ページ取得中 (attempt %d): %s", attempt + 1, url)
            ...
            logger.warning("HOME'S: WAF/CAPTCHA検知 (attempt %d): %s", attempt + 1, url)
        except Exception as e:
            logger.error("HOME'S: ページ取得エラー (attempt %d): %s — %s", attempt + 1, url, e)
        finally:
            page.close()

        if attempt < max_retries - 1:
            backoff = (15 * (2 ** attempt)) + random.uniform(2, 5)
            logger.info("HOME'S: %.1f秒待機後にリトライ...", backoff)
            time.sleep(backoff)

    logger.error("HOME'S: 全リトライ失敗: %s", url)
    return ""
...
def scrape_homes(max_pages: Optional[int] = 2, apply_filter: bool = True) -> Iterator[HomesListing]:
    ...
    try:
        ...
        empty_guard = EmptyParseGuard(HOMES_EMPTY_PARSE_TOLERANCE)
        ...
        while page <= limit:
            ...
            if not rows:
                if empty_guard.record_empty():
                    if total_parsed == 0:
                        ...
                        scraper_metrics.record("homes", empty_pages=empty_guard.consecutive)
                        dump_debug_html("homes", f"p{page}", html)
                        finish_reason = "empty_parse_abort"
                    else:
                        logger.info(f"HOME'S: ページ{page}で連続{empty_guard.consecutive}回0件パース（一覧の終端）")
                        finish_reason = "completed"
                    break
                logger.warning(
                    f"HOME'S: ページ{page}でパース0件 (HTML: {len(html)}B, 連続: "
                    f"{empty_guard.consecutive}/{HOMES_EMPTY_PARSE_TOLERANCE}) — 次ページへ進みます"
                )
            ...
            if empty_gap:
                ...
                scraper_metrics.record("homes", empty_pages=empty_gap)
