...
def normalize_listing_name(name: str) -> str:
    ...
    # ■□ 記号装飾を除去
    s = re.sub(r"[■□]+\s*", "", s)
    # ※...※ で囲まれた販促コピーを除去（「※池袋駅まで直通8分※○○」等）
    s = re.sub(r"※[^※]*※", "", s)
    # 閉じない ※ 以降も販促テキストとして除去
    s = re.sub(r"※.*$", "", s)
    # ～以降を除去
    s = re.sub(r"[~～].*$", "", s)
    # 「建物名/補足地名」の補足地名サフィックスを除去（SUUMO等のエリア注記）
    # 例:「サンクレイドルレヴィール池袋/上池袋２丁目」→「サンクレイドルレヴィール池袋」
    s = re.sub(r"\s*[/／].*$", "", s)
    # 末尾の広告文句
    ...
    # 日本語建物名の後に空白区切りで併記された英語（ローマ字）名を除去する。
    # HOME'S 等が「ブリリア有明スカイタワー Brillia有明SkyTower」のように和名＋英名を
    # 併記するため、和名だけが別表記化して重複検出を取りこぼしていた。
    # 直前が日本語（ひら/カタ/長音/漢字）かつ空白後が英字で始まる末尾を丸ごと落とす。
    # ※ 全体が英語名（先頭から英字）のケースは日本語文字が前に無いので対象外
    #   （THE TOYOSU TOWER 等は _KNOWN_NAME_ALIASES 側で処理）。
    s = re.sub(r"(?<=[ぁ-んァ-ヴー一-鿿])\s+[A-Za-z].*$", "", s).strip()
    # 空白をすべて除去（比較用）
    s = re.sub(r"\s+", "", s)
    ...
    s = s.replace("・", "")
    # カタカナの直後にある長音記号の異体字（ハイフン・各種ダッシュ・横棒・マイナス）を
    # カタカナ長音「ー」に統一する。「レヴィール / レヴィ―ル / レヴィ-ル」や、カタカナと
    # 漢字の境界（「ゼファー浅草 / ゼファ―浅草」U+30FC vs U+2015）の取りこぼし重複を吸収。
    # ※ U+30FC（ー）自体は対象外（[ァ-ヶ] は U+30F6 まで）。後続が英数字のとき（棟番号
    #   「コーポ-A」「メゾン-101」等）は分割記号なので変換しない。
    s = re.sub(r"(?<=[ァ-ヶ])[ｰ－‐‑‒–—―−\-]+(?![0-9A-Za-z])", "ー", s)
