...
## シナリオ適合分析（必須）
この物件がどのライフシナリオに最も適しているかを分析してください。
固定シナリオを採点するのではなく、物件の特性から「この物件ならこう活用するのが最適」を導き出すこと。

考慮すべきシナリオの例（該当するものを選択・組み合わせ）:
- DINKs期のステップアップ物件（3〜5年で売却）
- 子ども1〜2人で8〜10年居住→売却
- 子ども3人も想定した長期居住（10年超）
- 投資的短期保有（流動性・値上がり期待で5年以内売却）
- 終の住処候補（15年超の長期居住）

各シナリオについて:
1. この物件との適合度（適している / 条件次第 / 適さない）
2. 居住可能年数（間取り・面積・立地から何年まで快適か）
3. 出口シミュレーション（売却時期、その時点の想定残債、相場を踏まえた売却価格レンジ、損益見通し）
4. 前提条件やリスク（金利上昇、市況変動、管理費値上げ等の影響）

すべてのシナリオを列挙する必要はなく、この物件に関連性の高い2〜3シナリオに絞ること。
...
JSON形式で回答:
{
  ...
  "scenarios": [
    {"name": "子ども2人で8年居住→売却", "fit": "適している", "livable_years": 9, "exit_simulation": "2035年売却: 残債6,800万、想定売却9,000-9,500万。安全余白あり。", "risk": "金利2%超で月額+2万。修繕積立金値上げの可能性。"},
    {"name": "5年短期売却", "fit": "条件次第", "exit_simulation": "2031年売却: 残債7,600万、売却8,800-9,200万。余白薄い。", "risk": "市況悪化で残債割れリスク。指値が通れば安全圏。"}
  ],
...
def _build_score_context(listing: dict) -> str:
    ...
    mr = _parse_json_field(listing.get("mansion_review_data"))
    if mr and isinstance(mr, dict):
        _add("MR偏差値", mr.get("deviation_score"))
        _add("MR推定価格", mr.get("estimated_price_man"), "万円")
        _add("MR推定坪単価", mr.get("estimated_tsubo_price_man"), "万円/坪")
