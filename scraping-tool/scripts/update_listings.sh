...
#
# パイプライン構成:
#   Phase 1:  スクレイピング（中古のみ。新築は 2026-06 に全面廃止）
#   Phase 2a: 共有キャッシュ書き込み enricher（順次実行で競合回避）
#   Phase 2b: 読み取り専用 enricher（中古/成約実績の2トラック並列）
...
print_timing_summary() {
    ...
    for tsv_file in "$TIMING_DIR"/main.tsv "$TIMING_DIR"/phase2a.tsv "$TIMING_DIR"/track_a.tsv "$TIMING_DIR"/track_c.tsv "$TIMING_DIR"/phase2c.tsv; do
        [ -f "$tsv_file" ] || continue
        local label=""
        case "$tsv_file" in
...
print(f'is_new 注入完了: 中古 {new_c}/{len(cur)}件', file=sys.stderr)
...
# 2a-1. build_units_cache (中古)
_t=$(date +%s)
echo "build_units_cache (中古) 実行中..." >&2
python3 scripts/build_units_cache.py "${OUTPUT_DIR}/latest.json" || echo "build_units_cache に失敗しました（続行）" >&2
...
rm -f "${OUTPUT_DIR}/latest.json.backup"
