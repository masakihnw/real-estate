...
> **ジオコーディング最適化**: `geocode.py` は住所の早期フィルタ機能を持つ。他県住所（千葉・埼玉・神奈川等）や東京都多摩地域（八王子・町田・府中等）は Nominatim API 問い合わせ前にスキップし、不要な API コールと待機時間を削減する。キャッシュ検証ステップ（`validate_and_purge_cache()`）は不正エントリを自動削除して保存するため、手動メンテナンス不要。
