...
| **5行目** | ハザードバッジ | リスクがある場合のみ。洪水・土砂・液状化等を色付きバッジで表示 |
...
| パラメータ | UI | 範囲 | 刻み |
|-----------|-----|------|------|
| 金利 (%) | Stepper | 0.1 ~ 5.0 | 0.01 |
| 返済期間 (年) | Segmented Picker | 5, 10, 15, 20, 25, 30, 35, 40, 45, 50 | — |
| 頭金 (万円) | Slider | 0 ~ 物件価格 | 物件価格/100（最小10万円） |
...
| `ListingDetailView.swift` | 物件詳細のメイン画面。body を軽量化するため、各セクションを @ViewBuilder の private var に切り出している（delistedBanner, addressSection, commentSection, propertyInfoSection, commuteSection, hazardSection, sumaiSurfinSection, surroundingPropertiesSection, priceJudgmentsSection, externalLinksSection 等） |
...
| セクション | 内容 |
|-----------|------|
| **住まいサーフィン評価（中古）** | 沖式中古時価（実面積/70㎡換算）、中古値上がり率（未取得時は「—」表示）、割安判定バッジ、レーダーチャート（6軸偏差値）、駅・区ランキング |
| **周辺相場** | `hasSurroundingProperties` の場合のみ。周辺の中古マンション相場を表示（後述） |
| **販売価格割安判定** | `hasPriceJudgments` の場合のみ。物件の割安度を判定表示（後述） |
...
住まいサーフィンの中古物件ページで「販売価格が割安か判定する」ボタンをブラウザ自動化（Playwright）でクリックし、判定結果を取得する（`sumai_surfin_browser.py` `extract_chuko_price_judgments()`）。中古物件のみ対象。
...
| **新規検出** | 既存の `identityKey` に存在しない物件 → `isNew = true` + ローカル通知。同期ごとに既存物件の `isNew` をリセットし、新規挿入物件のみ `isNew = true` に設定 |
...
| 29 | 通勤時間計算 | 「通勤時間を計算する」ボタン | MKDirections で計算 → キャッシュ保存 |
...
| 47 | 駅レベル比較 | 閲覧 | 駅圏相場・物件との乖離率・トレンド・前年比を表示 |
| 48 | 同一マンション成約事例 | 閲覧 | 同一マンション候補の成約事例を最大5件表示 |
| 49 | 四半期推移チャート | 閲覧 | 区・駅の m²単価の四半期推移を折れ線チャートで表示 |
...
| 51 | 人口推移チャート | 閲覧 | 区の人口推移を折れ線+エリアチャートで表示 |
...
| # | 機能 | 操作 | 詳細 |
|---|------|------|------|
| 26 | 重要3項目 ON | ボタン | 洪水 + 液状化 + 建物倒壊 + 淡色地図を一括 ON |
...
> **Note**: `commute_enricher.py` は廃止済み。通勤時間は iOS アプリ側で Apple Maps（MKDirections）を使って正確に計算する方式に移行。
...
| **付与データ** | 区・駅レベルの成約 m² 単価、相場乖離率、前年比、四半期推移、同一マンション成約事例 |
| **キャッシュ構築** | `reinfolib_cache_builder.py` / `fetch_station_prices.py`（別ワークフローで実行） |
...
> **Note**: パイプラインでの通勤時間付与は廃止済み。iOS アプリ側で Apple Maps（MKDirections）を使って正確に計算する方式に移行。以下のツールは監査・検証用途で残存。
...
| `data/reinfolib_trends.json` | JSON | 不動産情報ライブラリ四半期推移キャッシュ |
...
| `isNew` | Bool | 前回同期時に存在しなかった新着フラグ（同期ごとにリセット） |
...
4. scripts/update_listings.sh --no-git
   ...
   ├── build_units_cache.py（総戸数・階数・権利形態キャッシュ更新）
