...
> **最終更新**: 2026-03-19（物件詳細の表示を .sheet → .fullScreenCover に変更し、iPad で全画面表示）
> **ステータス**: 運用中  
...
| コマンド | 動作 |
|---------|------|
| `./scripts/deploy.sh` | iOS ビルド番号インクリメント + アーカイブ + アップロード（フル実行） |
| `./scripts/deploy.sh --archive` | iOS ビルド番号インクリメント + アーカイブのみ |
| `./scripts/deploy.sh --upload` | 既存 iOS アーカイブのアップロードのみ |
| `./scripts/deploy.sh --mac` | Mac Catalyst ビルド番号インクリメント + アーカイブ + アップロード |
| `./scripts/deploy.sh --all` | iOS + Mac Catalyst 両方をアーカイブ + アップロード |
