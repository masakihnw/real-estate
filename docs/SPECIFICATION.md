...
| **5行目** | ハザードバッジ | リスクがある場合のみ。洪水・土砂・液状化等を色付きバッジで表示。東京都地域危険度はコンパクトな1ラベル（例: "建物倒壊3、火災4"）にまとめて他バッジと同じ高さに統一 |
...
| 51 | 人口推移チャート | 閲覧 | 区の人口推移を折れ線+エリアチャートで表示（monotone 補間） |
...
| **commute_enricher.py** | 駅名ベースのドアtoドア概算を `commute_info` として JSON に付与 | パイプラインで使用 |
