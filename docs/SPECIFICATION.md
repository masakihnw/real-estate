...
| ソース | 種別 | URL パターン | 状態 |
|--------|------|-------------|------|
| **SUUMO 中古** | 中古マンション | `suumo.jp/jj/bukken/ichiran/JJ012FC001/?ar=030&bs=011&ta=13&sc={ward_code}&kb={min}&kt={max}`（サーバーサイド価格フィルタ付き）。フィルタなし時は `suumo.jp/ms/chuko/tokyo/sc_{ward}/` | 有効 |
