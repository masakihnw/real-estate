...
extension Listing {
    ...
    func toAIConsultationPrompt(
        ...
        ## 推奨スペック（1軒目）
        - 価格: \(buyerProfile.budgetCriteriaInline())
        - 面積: 65㎡以上が本命、最低55㎡。55〜64㎡は間取り・駅力次第
        - 間取り: 2LDK+S〜3LDK、独立居室必須
        - 駅徒歩: 7分以内（理想5分以内）
        - 築年: 立地・管理を本質とし築30年程度まで許容（長期修繕計画・総会議事録・修繕積立金を確認）
        - 総戸数: 50戸以上（80〜200戸ベスト）、30戸未満は原則慎重
        - ランニング: 管理費+修繕月3.5万以内望ましい、4万超は慎重

        ## 金利耐性（必ず確認）
        金利上昇は賃金上昇を伴う前提で評価する。月額上限は「\(buyerProfile.monthlyLimitInline())」を基準とし、
        各金利シナリオの返済目安は予算シナリオに従う（実値は買い手プロフィールが正）。
        - 基準金利付近: 月額上限以内で成立するか
        - ターミナル1.5%: 賃金調整後の返済額で判断。ここで苦しいなら慎重
        - 本格引締め2.0%: 賃金調整後の返済額で判断。これで厳しいなら見送り
        - テールリスク2.5%: 即死しなければ十分
        - ストレス3.0%: 調整なし最悪想定（参考値）
        - 育休・時短期間中は賃金調整なし。月額上限で耐えられるか別途確認
    ...
    static func toAIComparisonPrompt(listings: [Listing], buyerProfile: BuyerProfile = .empty) -> String {
        ...
        ## 推奨スペック（1軒目）
        価格は \(buyerProfile.budgetCriteriaInline()) / 面積65〜72㎡（最低62㎡）/ 2LDK+S〜3LDK / 駅徒歩7分以内 / 築30年程度まで許容 / 総戸数50戸以上 / ランニング月3.5万以内

        ## 金利耐性
        金利上昇=賃金上昇の前提で評価。月額上限は「\(buyerProfile.monthlyLimitInline())」を基準とし、各金利シナリオの返済目安は予算シナリオに従う。基準金利→ターミナル1.5%→引締め2.0%→テール2.5%→ストレス3.0%の順に賃金調整後返済額で耐性を判断。育休・時短中は月額上限基準
