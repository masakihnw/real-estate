...
extension Listing {
    ...
    func toAIConsultationPrompt(otherCandidates: [Listing], buyerProfile: BuyerProfile = .empty) -> String {
        ...
        ## 推奨スペック（1軒目）
        - 価格: 9,300万〜1.03億（本命9,500万〜9,900万）。1.05億超は原則かなり慎重
        ...
        ## 金利耐性（必ず確認）
        金利上昇は賃金上昇を伴う前提で評価する。
        - 現行0.8〜1.1%: 月額上限26.7万円
        - ターミナル1.5%: 賃金調整後28〜29万円で判断。ここで苦しいなら慎重
        - 本格引締め2.0%: 賃金調整後29〜31万円で判断。これで厳しいなら見送り
        - テールリスク2.5%: 賃金調整後31〜33万円。即死しなければ十分
        - ストレス3.0%: 調整なし最悪想定（参考値）
        - 育休・時短期間中は賃金調整なし。26.7万円で耐えられるか別途確認
    ...
    static func toAIComparisonPrompt(listings: [Listing], buyerProfile: BuyerProfile = .empty) -> String {
        ...
        ## 推奨スペック（1軒目）
        価格9,300万〜1.03億 / 面積65〜72㎡（最低62㎡）/ 2LDK+S〜3LDK / 駅徒歩7分以内 / 築2006年以降 / 総戸数50戸以上 / ランニング月3.5万以内

        ## 金利耐性
        金利上昇=賃金上昇の前提で評価。現行0.8〜1.1%(上限26.7万) / ターミナル1.5%(28〜29万) / 引締め2.0%(29〜31万) / テール2.5%(31〜33万) / ストレス3.0%(調整なし)。育休・時短中は26.7万基準
