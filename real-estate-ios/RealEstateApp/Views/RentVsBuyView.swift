...
struct RentVsBuyView: View {
    let listing: Listing
    /// 価格・頭金・借入額・金利・期間は共有前提から。賃貸シナリオ固有の入力は @State 維持。
    ...
    init(listing: Listing, assumptions: LoanAssumptions) {
        self.listing = listing
        self.assumptions = assumptions
        self._propertyAppreciation = State(initialValue: listing.ssAppreciationRate ?? 0)
    }

    private var purchasePrice: Double { assumptions.purchasePriceYen }
    private var downPayment: Double { assumptions.downPaymentMan * 10000 }
    private var loanAmount: Double { assumptions.loanAmountYen }
    private var loanTermMonths: Double { Double(assumptions.loanYears) * 12 }

    private var monthlyLoanPayment: Double {
        let rate = assumptions.interestRatePercent / 100 / 12
    ...
    private var totalPurchaseCost: Double {
        let loanPayments = monthlyLoanPayment * min(simulationYears, Double(assumptions.loanYears)) * 12
    ...
    private var loanBalanceAtEnd: Double {
        let rate = assumptions.interestRatePercent / 100 / 12
        let totalMonths = loanTermMonths
        ...
        Group {
            ...
            Section("購入条件") {
                HStack {
                    Text("物件価格")
                    Spacer()
                    Text(Listing.formatPriceCompact(Int(assumptions.purchasePriceMan)))
