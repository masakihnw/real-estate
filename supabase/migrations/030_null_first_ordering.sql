-- 030: get_listings_for_ai の ORDER BY を未分析物件優先に変更
-- 背景: ai_prompt_hash が全モジュール共有のため、他モジュールによるハッシュ上書きが
-- 「偽のプロンプト不一致」を生み出し、未分析物件が LIMIT に到達できない問題の即時対応
...
COMMENT ON FUNCTION get_listings_for_ai IS 'Routine 用: module ごとに AI 分析が必要な物件を取得。未分析物件を優先的に返す';
