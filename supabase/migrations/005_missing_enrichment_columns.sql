-- Add missing enrichment columns that supabase_sync.py tries to write
-- but do not exist in the enrichments table.
-- Also fix price_history date format in listings_feed view.

-- 004 が未適用の場合に備えて AI recommendation カラムも含める
ALTER TABLE enrichments
    ...
    ADD COLUMN IF NOT EXISTS best_thumbnail_url TEXT,
    ADD COLUMN IF NOT EXISTS extracted_features JSONB,
    ADD COLUMN IF NOT EXISTS image_categories JSONB,
    ADD COLUMN IF NOT EXISTS dedup_confidence REAL,
    ADD COLUMN IF NOT EXISTS alt_sources JSONB,
    ADD COLUMN IF NOT EXISTS dedup_candidates JSONB,
    ADD COLUMN IF NOT EXISTS key_strengths JSONB,
    ADD COLUMN IF NOT EXISTS key_risks JSONB;

-- 既存ビューはカラム追加で CREATE OR REPLACE できないため DROP → CREATE
DROP VIEW IF EXISTS listings_feed CASCADE;

-- Recreate listings_feed view with:
--   1. New enrichment columns
--   2. price_history date format fixed (TIMESTAMPTZ → YYYY-MM-DD)
