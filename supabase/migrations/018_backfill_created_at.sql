-- created_at を first_seen_at からバックフィルし、ソート用インデックスを追加。
-- listings_feed ビューに created_at を追加。

-- (a) first_seen_at → created_at バックフィル
UPDATE listings
SET created_at = (first_seen_at || 'T00:00:00+00:00')::TIMESTAMPTZ
WHERE first_seen_at IS NOT NULL
  AND first_seen_at ~ '^\d{4}-\d{2}-\d{2}$';

-- (b) created_at でのソート用インデックス
CREATE INDEX IF NOT EXISTS idx_listings_created_at ON listings(created_at);

-- (c) 前提カラム追加（未適用マイグレーションのガード）
ALTER TABLE enrichments ADD COLUMN IF NOT EXISTS ai_recommendation_scenarios JSONB;
ALTER TABLE listings ADD COLUMN IF NOT EXISTS normalized_name TEXT NOT NULL DEFAULT '';
ALTER TABLE listing_sources ADD COLUMN IF NOT EXISTS consecutive_misses INTEGER DEFAULT 0;

-- (d) listings_feed ビュー再作成（l.created_at 追加）
DROP VIEW IF EXISTS listings_feed CASCADE;

CREATE VIEW listings_feed AS
SELECT
    l.id,
    l.identity_key,
    l.name,
    ...
    e.ai_recommendation_scenarios,
    e.investment_summary,
    e.highlight_badge,
    e.best_thumbnail_url,
    e.extracted_features,
    e.image_categories,
    e.dedup_confidence,
    e.alt_sources,
    e.dedup_candidates,
    ...
    (SELECT JSONB_AGG(JSONB_BUILD_OBJECT('date', TO_CHAR(ph.recorded_at, 'YYYY-MM-DD'), 'price_man', ph.price_man) ORDER BY ph.recorded_at)
...
-- (e) get_listings_since 再作成（CASCADE で消えたため、updated_at ベースのまま）
