-- 031: モジュール別 prompt_hash カラムの導入
-- 背景: 共有 ai_prompt_hash カラムが全モジュール間で上書きされ、
-- 「偽のプロンプト不一致」が大量発生し未分析物件が LIMIT に到達できない問題の根本解決。
-- 併せて investment_summary にも時間ベース再スコアリングを追加。

-- ============================================================
-- 1. モジュール別 prompt_hash カラムの追加
-- ============================================================
ALTER TABLE enrichments
  ADD COLUMN IF NOT EXISTS investment_summary_prompt_hash TEXT,
  ADD COLUMN IF NOT EXISTS text_enricher_prompt_hash TEXT,
  ADD COLUMN IF NOT EXISTS ai_scoring_prompt_hash TEXT,
  ADD COLUMN IF NOT EXISTS image_analyzer_prompt_hash TEXT;

COMMENT ON COLUMN enrichments.investment_summary_prompt_hash IS 'investment_summary モジュール固有の prompt_hash';
COMMENT ON COLUMN enrichments.text_enricher_prompt_hash IS 'text_enricher モジュール固有の prompt_hash';
COMMENT ON COLUMN enrichments.ai_scoring_prompt_hash IS 'ai_scoring モジュール固有の prompt_hash';
COMMENT ON COLUMN enrichments.image_analyzer_prompt_hash IS 'image_analyzer モジュール固有の prompt_hash';

-- ============================================================
-- 2. 既存データのバックフィル
-- 結果カラムが存在する物件に現在のアクティブ prompt_hash をセットし、
-- マイグレーション直後の不要な再処理を防止する。
-- ============================================================
UPDATE enrichments e
SET investment_summary_prompt_hash = (
  SELECT ap.prompt_hash FROM ai_prompts ap
  WHERE ap.module = 'investment_summary' AND ap.is_active = true LIMIT 1
)
WHERE e.ai_recommendation_score IS NOT NULL;

UPDATE enrichments e
SET text_enricher_prompt_hash = (
  SELECT ap.prompt_hash FROM ai_prompts ap
  WHERE ap.module = 'text_enricher' AND ap.is_active = true LIMIT 1
)
WHERE e.extracted_features IS NOT NULL;

UPDATE enrichments e
SET ai_scoring_prompt_hash = (
  SELECT ap.prompt_hash FROM ai_prompts ap
  WHERE ap.module = 'ai_scoring' AND ap.is_active = true LIMIT 1
)
WHERE e.ai_listing_score IS NOT NULL;

UPDATE enrichments e
SET image_analyzer_prompt_hash = (
  SELECT ap.prompt_hash FROM ai_prompts ap
  WHERE ap.module = 'image_analyzer' AND ap.is_active = true LIMIT 1
)
WHERE e.image_categories IS NOT NULL;

-- ============================================================
-- 3. upsert_ai_enrichment をモジュール別 prompt_hash 対応に更新
-- ============================================================
...
-- ============================================================
-- 4. get_listings_for_ai をモジュール別 prompt_hash + null-first + rescore 対応に更新
-- ============================================================
...
COMMENT ON FUNCTION get_listings_for_ai IS 'Routine 用: module ごとに AI 分析が必要な物件を取得。モジュール別 prompt_hash で正確な再処理判定。未分析物件を優先的に返す';

-- ============================================================
-- 5. investment_summary の config にデフォルト再スコア設定を追加
-- ============================================================
UPDATE ai_prompts
SET config = COALESCE(config, '{}'::jsonb) || jsonb_build_object(
  'max_items_per_run', COALESCE((config->>'max_items_per_run')::int, 20),
  'rescore_min_score', 4,
  'rescore_interval', '1 day'
)
WHERE module = 'investment_summary' AND is_active = true;
