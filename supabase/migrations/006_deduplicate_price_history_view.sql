-- Fix price_history_json duplication in listings_feed view.
-- Problem: multiple sources (suumo, rehouse, etc.) each record price_history
-- for the same listing, causing interleaved entries with different prices.
-- Fix: show only the primary source's history, one record per day.
