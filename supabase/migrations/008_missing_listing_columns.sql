-- Add missing columns that exist in JSON but not in Supabase tables.
-- Also update listings_feed view to include all fields.

-- listings テーブル: スクレイパー由来のメタデータ
ALTER TABLE listings
    ADD COLUMN IF NOT EXISTS first_seen_source TEXT,
    ADD COLUMN IF NOT EXISTS geocode_confidence TEXT,
    ADD COLUMN IF NOT EXISTS geocode_fixed BOOLEAN DEFAULT FALSE,
    ADD COLUMN IF NOT EXISTS alt_urls JSONB;

-- enrichments テーブル: finalize 計算結果
ALTER TABLE enrichments
    ADD COLUMN IF NOT EXISTS is_cheapest_in_building BOOLEAN,
    ADD COLUMN IF NOT EXISTS competing_price_range TEXT;

-- listings_feed ビューを再定義（新カラム + listing_sources の listing_agent/is_motodzuke を追加）
